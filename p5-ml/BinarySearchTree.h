...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int height_impl(const Node *node) {
    if (empty_impl(node)) return 0;
      ...
      int l = height_impl(node->left)+1;
      int r = height_impl(node->right)+1;
      if (l > r) return l;
      else return r;
    }
  }
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    if (!node) {
      ...
      if (node->left == nullptr) {
        ...
        ist->right = nullptr;
        node->left = ist;
      } else insert_impl(node->left, item, less);
    }
    return node;
  }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    if (!node) return nullptr;
    else if (!less(val, node->datum)) return min_greater_than_impl(node->right,val,less);
    else if (less(val, node->datum)) {
      Node* tm = min_greater_than_impl(node->left, val, less);
      if (!tm) return node;
      else return tm;
    }
    else return node;
